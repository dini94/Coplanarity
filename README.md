...
A typical donor–π-spacer–acceptor (D–π–A) dye has two rotatable junctions: where the
donor connects to the π-spacer (φ₁), and where the π-spacer connects to the acceptor
(φ₂). The tool's in-app diagram shows exactly where these are measured. Enter both
(plus any additional internal torsions, e.g. if the spacer itself isn't fully fused)
as named junction rows, and the tool returns a single quantitative coplanarity value —
plus a separate, complementary whole-molecule **planarity** check (MPP/SDP) for when
full atomic coordinates are available.

### Terminology
- **Coplanarity** — the term consistently used in DSSC/D–π–A dye papers for
  dihedral-angle-based assessments of whether linked donor/spacer/acceptor units
  share a plane. This is what the dihedral junction inputs below compute.
...
typical D–π–A case (n=2):  C = [cos²(φ₁)·cos²(φ₂)]^(1/2)
general (n junctions):     C = [c₁·c₂·...·cₙ]^(1/n)             (geometric mean)
```
...
through every junction in sequence — donor→spacer, then spacer→acceptor — so one
badly-twisted junction should dominate the whole-molecule score the way a weak link
dominates a chain. Geometric mean does exactly that — a single 90° junction collapses
the whole product to 0 — whereas an arithmetic mean would just dilute it.
...
### Entering junctions
Each molecule gets a list of named dihedral rows — a junction label (e.g.
"Donor–π-spacer") and its angle in degrees. New custom molecules default to the two
junctions every D–π–A dye has; use "+ Add junction" for additional internal torsions
(e.g. a non-fused two-ring spacer has three junctions total: donor–spacer1,
spacer1–spacer2, spacer2–acceptor). The per-junction breakdown table and the
"weakest junction" diagnostic in the metrics card both report by junction name, so
it's immediately clear which bond is limiting a dye's coplanarity.
...
and doesn't have a single D–π–A backbone the way the fluorene dyes do (no dihedral
junctions are pre-filled for it). Use the atom-range field to restrict the fit to one
conjugated wing for a more chemically meaningful comparison.

No geometry or dihedral data is preloaded yet for FPCN/FTCN/FFCN — paste your own
DFT-optimized coordinates or measured dihedral angles via "+ Custom molecule" and
they'll be computed and compared alongside N719.
...
├── coplanarity.html    ← Tool 1: coplanarity (dihedral junctions) + planarity (MPP/SDP)
└── README.md           ← this file
```
